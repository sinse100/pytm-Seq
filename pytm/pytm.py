...
## (modified) - pluggin to analyze attack scenario
...
to a boolean True or False""",
    ...
    ## (근우 메모)
    ## apply : 인자로 전달된 DFD 요소(target)에 현재 위협(self)관련 패턴이 있는지 확인하여 bool 타입으로 반환
    ## @param
    ##  - self : 현재 패턴을 확인하고자 하는 위협 
    ##  - target : 현재 위협 패턴(self.condition, self.target)과의 부합 여부를 확인할 DFD 요소
    def apply(self, target):
        ## (근우 메모) 확인하고자 하는 DFD 요소 타입과 위협 패턴에 정의된 타입이 일치하는지 확인
        if not isinstance(target, self.target):
            return None
        ## (근우 메모) 인자로 전달된 DFD 요소에서 위협 패턴이 존재하는지 확인 
...
with same properties, except name and notes""",
    ...
    def resolve(self,mode = None):
        ...
        ## (근우 메모) 
        ##  - DFD 상의 모든 요소(e)에 대해, 각 요소가 정의된 위협 패턴(threatlib/threats.json에 정의)에 부합하는지 검토 
        for e in TM._elements:
            ...
            for t in TM._threats:
                ## (근우 메모) DFD 상의 어떤 요소(e)가 위협의(t) 패턴에 부합하는지 확인
            ...
            ## (modified) - analyze attack scenario starting from nodes('starts') on the given dfd
            '''
            if starts is not None:    
                tp_engine = ThreatPathEngine(self,"./pytm/threatlib/scenario.json")
                tp_engine.check_type_of_asset()
                result = tp_engine.find_all([starts])
