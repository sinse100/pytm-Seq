...
class ThreatPathEngine:
    def __init__(self, threat_model,scenarios_path):
        ...
        for scenario in scenario_json:
            attack_steps = []
            last_steps = []
            for attack_step in scenario['phase_except_last']:
            ...
            for last_step in scenario["phase_last"]:
                pattern = VulnerabilityPattern(
                    src_function_type=last_step["src"][0],
                    src_element_type=last_step["src"][1],
                    dst_function_type=last_step["dst"][0],
                    dst_element_type=last_step["dst"][1],
                )
                last_steps = last_steps + [pattern]
            attack_scenarios = attack_scenarios + [AttackScenarioSpec(scenario_id=scenario["id"], scenario_name=scenario["name"],last = last_steps,sub_phases=attack_steps)]
    ...
    def _search_from(self,
        ...
        ## if there is no outbound dataflows to continue go back
        if len(ordered_outflows) == 0 :         
            return current_traces  

        for flow in ordered_outflows :
            ## check whether this flow is visited or not (if visited mark the flag(skip_this_flow))
            skip_this_flow = False
            for trace in current_traces:
                if trace == flow :
                    skip_this_flow = True
            
            ## if dataflow is not visited, check whether the dataflow is mathced to one of patterns
            if skip_this_flow == False:
                for patt in patterns :
                    if (patt[0].src_element_type==str(current_node.__class__.__name__) and patt[0].src_function_type == current_node.function_type 
                        ...
                        ## follow through the next Dataflow
                        current_traces = current_traces + [flow] 
                        patt[1] = True
                        ##self.recur_idx = self.recur_idx + 1
