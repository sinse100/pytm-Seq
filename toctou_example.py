...
## Process
process1 = Process("Database Read")
...
db_to_process2 = Dataflow(db,process2, "Original User Data")
process1_to_db.order = 2


User =ExternalEntity("User")
coupon_db = Datastore("Shared Database")
user_db = Datastore("User Account")
static_file_storage = Datastore("Static File(.js, .css, .html) Storage")
product_db = Datastore("Product Database")
login_process = Process("Login")
register_process = Process("Register")
web_server = Process("Web Server")


# Coupon Code Checker : 사용자가 입력한 쿠폰 코드가 이미 사용된 것인지 확인하는 로직
coupon_code_checker = Process("Coupon Code Checker")
# Coupon Code Redeemer : 사용자가 입력한 쿠폰 코드가 만료되었 로직 
coupon_code_redemmer = Process("Coupon Code Redeemer")
# File Meta-Data Reader ls 바이너리와 같이  파일의 메타데이터(inode)를 읽어와 파일 종류, 소유자, 권한, 크기, 수정 시간 등을 조회
product_purchase = Process("Product Purchase")
...
# 참고-1 : https://fdzdev.medium.com/guide-to-identifying-and-exploiting-toctou-race-conditions-in-web-applications-c5f233e32b7f
# 참고-2 : https://hackerone.com/reports/157996#:~:text=Coupons%20hackerone,times%2C%20and%20stacking%20savings%20added
